...
export function AdminOutboxPage() {
  ...
  useEffect(() => {
    api.get<{ emails: Email[] }>("/api/admin/outbox").then((data) => setEmails(data.emails));
  }, []);

  return (
    <div className="space-y-6">
      <h1 className="font-display text-4xl">{t("admin.outbox")}</h1>
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-sand">
            <tr>
              ...
              <th className="px-4 py-3">{t("admin.to")}</th>
              <th className="px-4 py-3">{t("admin.subject")}</th>
              <th className="px-4 py-3">{t("admin.emailStatus")}</th>
            </tr>
          </thead>
          <tbody>
            {emails.map((item) => (
